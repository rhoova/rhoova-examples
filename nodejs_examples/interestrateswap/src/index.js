    ...
    createTask.addEventListener("submit", (event)=>{
        ...
        arrayData.forEach((data) => {
            ...
            if(data.name.includes("apiKey")){
				...
				forecastval.forEach(forecastValue => {
				  if (jsonData[forecastValue])
				    data.value = JSON.stringify(jsonData[forecastValue])
				})
				...
				discval.forEach(discountValue => {
				  if (jsonData[discountValue])
				    data.value = JSON.stringify(jsonData[discountValue])
				})
        ...
        client.createTask({data: taskData, calculationType: rhoova.CalculationType.IRS, waitResult: true}).then((result) => {
            ...
            if(result.error){
                ...
                let tPropertiesBody ='<td>'+resultData.pv+'</td><td>'+resultData.fixedLegPv+'</td><td>'+resultData.floatingLegPv+'</td>' +
                    '<td>'+resultData.PV01+'</td><td>'+resultData.DV01+'</td><td>'+resultData.fairRate+'</td>' +
                    '<td>'+resultData.fairSpread+'</td><td>'+resultData.spread+'</td><td>'+resultData.impliedQuote+'</td>'
		...
		client.createTask({data: yieldcurveData, calculationType: rhoova.CalculationType.YIELD_CURVE, waitResult: true}).then((result) => {
            let resultData = JSON.parse(result.result);
            let yValues = [];
            let xValues  = [];
